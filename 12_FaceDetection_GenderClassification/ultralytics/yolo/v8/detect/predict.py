...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    classifier = GenderClassifier("gender_detection.model")
...
class DetectionPredictor(BasePredictor):
    ...
    def write_results(self, idx, preds, batch):
        ...
        for c in det[:, 5].unique():
            ...
            aligns=im0.shape
            if count_of_object > 5:
                text = "Over-Crowded"
                #align_bottom variable represents the height of the image
                align_bottom=aligns[0]/14
                #align width variable represnts the width of the image
                align_right=(aligns[1]/2.5)
                cv2.line(im0, (int(align_right),int(align_bottom-10)), (int(align_right+210),int(align_bottom-10)), (0,0,255), 40) 
                cv2.putText(im0,text,(int(align_right),int(align_bottom)),cv2.FONT_HERSHEY_SIMPLEX,1, (255,255,255),2,cv2.LINE_AA)
